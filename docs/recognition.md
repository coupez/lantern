...
Commissionable and commissioner advertisements can supply a UTF-8 `DN` name (up to 32 bytes), decimal `VP` vendor/product identifiers, and a decimal `DT` device-type identifier. TXT keys are case-insensitive. Numbers must fit their protocol widths without signs, spaces, or leading zeros. Malformed values do not create identity claims; valid unknown numbers remain visible in claims and raw advertisements. A vendor ID alone is supported. The IDs do not establish a manufacturer or retail model, and a DNS hostname or service instance does not establish a link-layer MAC.
...
Source URLs, input hashes, license, and selection rules accompany the data. `python3 scripts/build-matter-types.py --source-dir <download-directory>` regenerates it from the six hash-checked source files listed there; save each downloaded file under its basename. Upstream notices and license accompany releases. Controlled split-reply IPv4/IPv6 socket fixtures cover all three services; physical Matter devices, Thread border-router forwarding, sleepy-node availability, and vendor/product-name catalogs remain unverified or unimplemented.
