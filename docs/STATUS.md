...
2. Improve discovery coverage and speed further: privileged macOS ARP/NDP runtime verification, broader IPv6 real-device coverage and recovery when a selected local IPv6 address is still tentative, broader pacing/retry validation under sustained network pressure, and benchmark/accuracy comparisons across real device networks. The current quick scan is fast but can miss filtered/sleeping devices.
