...
5. Public release publication. GitHub API checks on 2026-09-08 confirm `coupez/lantern` is private and has no release assets. The current integration is pushed and under review in draft PR #1. Canonical installation from private GitHub with authentication, core imports from another module, staged installation, and self-contained local archives are verified; unauthenticated public retrieval remains unverified until repository/release visibility is resolved.
