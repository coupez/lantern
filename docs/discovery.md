...
One ephemeral UDP socket handles the entire pass alongside the existing discovery methods. Requests use independent random transaction IDs, with 32 requests per burst and a 10 ms pause between bursts. Writes have a 2 ms deadline. Timeout, queue-pressure and peer-route failures do not stop the first pass; socket/deadline failures and short writes do. The first pass has a send budget of two seconds plus 10 ms for each rounded-up group of 32 unique targets, preventing slow writes from multiplying across a large subnet indefinitely.

Pressure-failed targets receive at most one retry, with 2 ms pacing and a shared budget of the smaller of `--timeout` and 100 ms. Successfully sent but unanswered probes are not retransmitted. Recovered send failures do not produce an incomplete warning; remaining failures and unattempted targets do, with bounded counts and one representative cause. The full `--timeout` response window starts after the final write; fully answered target sets finish immediately. Cancellation closes the socket and keeps collected responses. Attempted addresses contribute to report-level `probed`, including failed writes, without double-counting retries. These bounds improve scheduling fairness; they cannot guarantee delivery under persistent host/network pressure.
