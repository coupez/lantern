...
Banner parsing assembles fragmented TCP lines and bounds each response to 8 KiB, each line to 2 KiB, and HTTP/SSH inspection to 64 lines including the first line. HTTP uses the first Server header, falling back to the status line; body text is never interpreted as a header. Oversized or interrupted lines are discarded. Terminal and directional controls are removed from returned text. SSH permits preamble lines before its identification ([RFC 4253 §4.2](https://www.rfc-editor.org/rfc/rfc4253.html#section-4.2)); the collector prefers a line beginning literally with `SSH-`. If none arrives before EOF, an error, or a collection limit, the first line remains a diagnostic fallback. A partial identification at EOF retains the previous display behavior. Returned text is a banner observation, not strict SSH syntax validation or authenticated software identity. No client SSH greeting, key exchange, or authentication is sent. These limits intentionally keep banner collection lightweight; a missing banner does not mean an open service is unavailable.
