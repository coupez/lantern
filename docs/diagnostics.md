...
Multicast checks use the same source-address fallback as scans. If binding rejects a tentative, failed-DAD, or removed address, they try another matching address on the selected interface within the original deadline. Successful output names the usable source; permission/configuration failures and exhaustion remain visible. Opening the socket still sends no discovery packets.
