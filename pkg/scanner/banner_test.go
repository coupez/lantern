...
func TestBannerResponse(t *testing.T) {
	for _, tc := range []struct{ name, service, input, want string }{
		...
		{"SSH preamble", "ssh", "Authorized use only\r\nMaintenance window notice\r\nSSH-2.0-Lantern\r\n", "SSH-2.0-Lantern"},
		{"SSH empty preamble and LF", "ssh", "\nNotice\nSSH-1.99-Lantern\n", "SSH-1.99-Lantern"},
		{"SSH preamble then EOF identification", "ssh", "Notice\r\nSSH-2.0-Lantern", "SSH-2.0-Lantern"},
		{"SSH error without identification", "ssh", "Access denied\r\nTry later\r\n", "Access denied"},
		{"SSH prefix must start line", "ssh", "Notice\r\n SSH-2.0-Misleading\r\nSSH-2.0-Lantern\r\n", "SSH-2.0-Lantern"},
		{"SSH preamble line budget", "ssh", strings.Repeat("Notice\r\n", 64) + "SSH-2.0-Hidden\r\n", "Notice"},
		{"SSH last allowed line", "ssh", strings.Repeat("Notice\r\n", 63) + "SSH-2.0-Lantern\r\n", "SSH-2.0-Lantern"},
		{"SSH preamble byte budget", "ssh", strings.Repeat(strings.Repeat("a", 1022)+"\r\n", 8) + "SSH-2.0-Hidden\r\n", strings.Repeat("a", 1022)},
		{"SSH overlong preamble line", "ssh", "Notice\r\n" + strings.Repeat("a", 3000) + "\r\nSSH-2.0-Hidden\r\n", "Notice"},
