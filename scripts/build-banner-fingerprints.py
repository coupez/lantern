#!/usr/bin/env python3
"""Import the pinned BSD-2-Clause Recog SSH/HTTP/FTP/SMTP/IMAP/POP3 data without executing upstream code."""
import argparse
import gzip
import hashlib
import io
...
FILES = {
    # Source order within each field remains authoritative.
    ...
    'xml/imap_banners.xml': '21710f2207dd001b3e4a8fccf1adf45f11d7dfcecb18f30c217c91ae720fa199',
    'xml/pop_banners.xml': '7823a1c6783402a8d6081adf996ac73f8331e50b899ec403455a125140921b2a',
}
...
def extract(data):
    ...
    if root.tag != 'fingerprints' or root.get('matches') not in ('ssh.banner', 'http_header.server', 'ftp.banner', 'smtp.banner', 'imap4.banner', 'pop3.banner'):
    ...
    for number, (node, line) in enumerate(zip(root, lines)):
        ...
        greeting = root.get('matches') in ('ftp.banner', 'smtp.banner', 'imap4.banner', 'pop3.banner')
...
def main():
    ...
    mail = json.loads((ROOT / 'research/mail-access-catalog-review.json').read_text())
    if mail['catalog_revision'] != REVISION:
        raise ValueError('Mail source revision differs')
    sources = reference['sources'] + mail['sources'] + [e for e in greetings['sources'] if e['path'] in FILES and e['path'] not in {s['path'] for s in reference['sources']}]
    ...
    manifest = {'source': 'Rapid7 Recog', 'revision': REVISION, 'license': 'BSD-2-Clause', 'sources': required,
                ...
                'selection': 'All pinned SSH, HTTP Server, FTP, SMTP, IMAP and POP3 greeting patterns, in upstream order. FTP/SMTP/IMAP/POP3 model Ruby line anchors and supported XML case/dot-newline flags; SSH/HTTP expressions are unchanged. Parameters, certainty, preference, and source lines retained. Examples are test data only.'}
