...
docker run --rm --platform "$platform" -e "LANTERN_EXPECT_GOARCH=$arch" --cap-drop ALL --cap-add NET_ADMIN "$image" sh -c 'go build -o /tmp/lantern ./cmd/lantern && python3 /usr/local/bin/test-neighbor-interfaces.py /tmp/lantern && python3 /usr/local/bin/test-ndp-linux.py /tmp/lantern --without-raw && python3 /usr/local/bin/test-wsdd.py /tmp/lantern --ipv6'
