...
docker run --rm --platform "$platform" -e "LANTERN_EXPECT_GOARCH=$arch" --cap-drop ALL "$image" sh -c 'go build -o /tmp/lantern ./cmd/lantern && python3 /usr/local/bin/test-doctor.py /tmp/lantern --expect-arp unavailable'
docker run --rm --platform "$platform" -e "LANTERN_EXPECT_GOARCH=$arch" --cap-drop ALL --cap-add NET_ADMIN "$image" sh -c 'go build -o /tmp/lantern ./cmd/lantern && python3 /usr/local/bin/test-neighbor-interfaces.py /tmp/lantern && python3 /usr/local/bin/test-ndp-linux.py /tmp/lantern --without-raw'
